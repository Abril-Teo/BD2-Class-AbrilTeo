...
insert into customer(store_id,address_id, first_name, last_name, email) 
			values(1,(select max(address_id) 
                        ...
                        where co.country='United States'), 'Teo', 'Abril', 't.abril@gmail.com');
...
insert into rental (rental_date,inventory_id, staff_id, customer_id) 
			values( now(), (select inventory_id 
							from inventory i inner join film f on f.film_id=i.film_id 
                            where f.title='HOLY TADPOLE' LIMIT 1),
							(select max(staff_id) 
                            from staff where store_id=2),
							3
							);
...
delete from rental where inventory_id in (select inventory_id from inventory where film_id=4);
