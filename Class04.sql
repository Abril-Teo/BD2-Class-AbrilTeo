...
#1. SHOW title and special features of films that are PG-13
SELECT title,special_features FROM film
WHERE rating = 'PG-13';

#2. get a list of all the different films duration
select length from film group by length ORDER BY length ASC;
...
#4. show title, category rating of films that have 'Behind the Scenes' as special_features
...
#5. show first name and lastname of actors that acted in 'ZOOLANDER FICTION'
...
#8. get all the films that are avaible in store id 2 and the manager first/last name of thsi store(the manager will appear in all the rows)
